...
public class ActivityController {
    ...
    public void fileDownload(HttpServletResponse response) throws Exception {
        ...
        // attachment即下载。mystudentList.xls为下载后的文件名
        response.addHeader("Content-Disposition", "attachment;filename=mystudentList.xls");

        //读取excel文件(InputStream)，把输出到浏览器(OutoutStream)
        InputStream is = new FileInputStream("C:\\Users\\惜听\\Documents\\BaiduYun\\实战项目\\CRM项目（SSM框架版）\\studentList.xls");
    ...
    public void exportAllActivitys(HttpServletResponse response) throws Exception {
        ...
        // 第一种写法如下（效率较低）：内存数据写入磁盘，读取磁盘数据再显示到页面=======
//        //根据wb对象生成excel文件
//        OutputStream os = new FileOutputStream("C:\\Users\\惜听\\Documents\\BaiduYun\\实战项目\\CRM项目（SSM框架版）\\activityList.xls");
//        wb.write(os);
//        //关闭资源
//        os.close();
//        wb.close();
...
//        OutputStream out = response.getOutputStream();
//        InputStream is = new FileInputStream("C:\\Users\\惜听\\Documents\\BaiduYun\\实战项目\\CRM项目（SSM框架版）\\activityList*.xls");
//        byte[] buff = new byte[256];
//        int len = 0;
//        while ((len = is.read(buff)) != -1) {
//            out.write(buff, 0, len);
...
//        out.flush();
        ...
        response.addHeader("Content-Disposition",
                "attachment;filename=activityList.xls");
        OutputStream out = response.getOutputStream();
    ...
    Object fileUpload(String userName, MultipartFile myFile) throws Exception {
        ...
        File file = new File("C:\\Users\\惜听\\Documents\\BaiduYun\\实战项目\\CRM项目（SSM框架版）", originalFilename);//路径必须手动创建好，文件如果不存在，会自动创建
    ...
    Object importActivity(MultipartFile activityFile, String userName, HttpSession session) {
        ...
        try {
            ...
            //InputStream is=new FileInputStream("C:\Users\惜听\Documents\BaiduYun\实战项目\CRM项目（SSM框架版）\\"+originalFilename);
